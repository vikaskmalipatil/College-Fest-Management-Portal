  ...
  useEffect(() => {
    ...
    const registerForEvent = async (eventId) => {
      ...
      try {
        ...
        const localBranch = (localStorage.getItem("branch") || "")
          .toString()
          .toUpperCase();
    ...
    const unregisterFromEvent = async (eventId) => {
      ...
      try {
        ...
        const deletes = snap.docs.map((d) =>
          deleteDoc(doc(db, "registrations", d.id))
        );
    ...
    const handleLogout = async () => {
      ...
      // 2. Clear state to stop Firestore calls
      setUser(null);
      setEvents([]);
      setRegisteredEventIds(new Set());
        ...
        <main className="max-w-6xl mx-auto px-6 pt-10">
          <div className="mb-10">
            ...
            <p className="text-slate-500 mt-1">
              Exclusive events curated for{" "}
              <span className="text-indigo-600 font-bold">{branch}</span>
            </p>
          </div>
                  ...
                  <div
                    ...
                    className={`group relative bg-white border ${
                      isReg
                        ...
                        : "border-slate-200"
                    } rounded-[2.5rem] p-8 transition-all hover:shadow-2xl hover:shadow-indigo-100 hover:-translate-y-1 overflow-hidden`}
                  >
                    {/* Status Badge */}
          ...
          {!loading && events.length === 0 && (
            <div className="text-center py-24 bg-white rounded-[3rem] border border-dashed border-slate-300">
              ...
              <p className="text-slate-400">
                Check back later for updates to the {branch} branch.
              </p>
            </div>
          )}
        </main>
